...
router.get('/count', async (req, res) => {
    try {
        ...
        res.status(500).json({ mensagem: 'Erro ao contar documentos', erro: err.message });
    }
});
...
router.get('/summary', async (req, res) => {
    try {
        ...
        res.status(500).json({ mensagem: 'Erro ao gerar resumo', erro: err.message });
    }
});

// Obter todos os ciclos de faturamento
router.get('/', async (req, res) => {
    try {
        const billingCycles = await BillingCycle.find();
        res.json(billingCycles);
    } catch (err) {
        res.status(500).json({ mensagem: 'Falha ao buscar ciclos de faturamento', erro: err.message });
    }
});
